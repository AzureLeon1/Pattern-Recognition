...
def single_sample_bp(features, labels, network: MLP, max_step=100000, learning_rate=0.1, theta=1e-5):
    ...
    # print("error", error)
    # print("step", max_step)
    # evaluate(features, labels, network)
    return network, err


def batch_bp(features, labels, network: MLP, max_batch=5000, learning_rate=0.1, theta=1e-5):
    """批量更新算法"""
    err_next = 10000
    err = []
    d1 = np.zeros_like(network.w_i_h)
...
if __name__ == "__main__":
    ...
    list_acc_single_sample_bp = []
    list_acc_batch_bp = []
    for i in range(k):
        features = features.reshape(3, 10, -1)
        labels = labels.reshape(3, 10, -1)
        if i == 0:
            train_features = features[:, 2 * i + 2:, :].reshape(-1,feature_dim)
            test_features = features[:, 2 * i:2 * i + 2, :].reshape(-1, feature_dim)
            train_labels = labels[:, 2 * i + 2:, :].reshape(-1, 1)
            test_labels = labels[:, 2 * i:2 * i + 2, :].reshape(-1, 1)
        elif i == k-1:
            train_features = features[:, : 2 * i, :].reshape(-1, feature_dim)
            test_features = features[:, 2 * i:2 * i + 2, :].reshape(-1, feature_dim)
            train_labels = labels[:, : 2 * i, :].reshape(-1, 1)
            test_labels = labels[:, 2 * i:2 * i + 2, :].reshape(-1, 1)
        else:
            train_features = np.vstack([features[:, : 2*i, :].reshape(-1, feature_dim), features[:, 2*i+2:, :].reshape(-1, feature_dim)])
            test_features = features[:, 2*i:2*i+2, :].reshape(-1, feature_dim)
            train_labels = np.vstack([labels[:, : 2*i, :].reshape(-1, 1), labels[:, 2*i+2:, :].reshape(-1, 1)])
            test_labels = labels[:, 2*i:2*i+2, :].reshape(-1, 1)

        # train_features = train_features.reshape(24, -1)
        # test_features = test_features.reshape(6, -1)
        # train_labels = train_labels.reshape(24, -1)
        # test_labels = test_labels.reshape(6, -1)

        network, y1 = single_sample_bp(train_features, train_labels, network, learning_rate=lr)
        acc = evaluate(test_features, test_labels, network)
        print('No.{} fold Accuracy (Single Sample BP): {:.2%}'.format(i+1, acc))

        network2, y2 = batch_bp(train_features, train_labels, network2, learning_rate=lr)
        acc2 = evaluate(test_features, test_labels, network2)
        print('No.{} fold Accuracy (Batch BP): {:.2%}'.format(i+1, acc2))


        list_acc_single_sample_bp.append(acc)
        list_acc_batch_bp.append(acc2)

    print('')
    print('======= Overall Evaluation =======')
    print('5-Fold Accuracy (Single Sample BP): {:.2%}'.format(np.mean(list_acc_single_sample_bp)))
    print('5-Fold Accuracy (Batch BP): {:.2%}'.format(np.mean(list_acc_batch_bp)))




    # network, y1 = single_sample_bp(features, labels, network, learning_rate=lr)
    #
    # network2, y2 = batch_bp(features, labels, network2, learning_rate=lr)
    # node1 = []
    # node2 = []
